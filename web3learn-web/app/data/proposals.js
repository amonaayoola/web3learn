  ...
  modules: [
    ...
    // MODULE 1: ETHEREUM PROPOSALS
    {
      ...
      quiz: [
        ...
        { id: 'proposals_eth_q6', question: 'A developer wants to propose a change to how the EVM handles a specific opcode. Which proposal type should they file, and why?', options: ['An ERC, because opcode behavior is an application-layer concern', 'An EIP, because it changes the core protocol and requires node operators to upgrade', 'Neither, opcode changes are handled internally by the Ethereum Foundation', 'An ERC, because opcodes affect token contracts'], correct: 1, explanation: 'EIPs propose changes to the Ethereum protocol itself, including how the EVM handles opcodes. This requires node operators and validators to upgrade their software, unlike ERCs which define application-layer standards.' },
        { id: 'proposals_eth_q7', question: 'A new wallet wants to display every token a user holds without writing custom integration code for each one. What makes this possible?', options: ['Every token pays a listing fee to be indexed', 'Every ERC-20 token implements the same six functions, so the wallet can query balanceOf on any of them', 'Ethereum maintains a centralized token registry', 'Tokens broadcast their balances to a public API'], correct: 1, explanation: 'Because ERC-20 standardizes totalSupply, balanceOf, transfer, transferFrom, approve, and allowance, any wallet can call balanceOf on any ERC-20 contract using the same code, with no per-token integration needed.' },
        { id: 'proposals_eth_q8', question: 'A user mints an NFT, and the marketplace shows a broken image even though the contract confirms they own token ID 88. What is the most likely cause?', options: ['The NFT was burned automatically', 'The tokenURI metadata, often hosted off-chain on IPFS, is missing or unreachable', 'ERC-721 does not support images', 'The user does not actually own the token despite what the contract says'], correct: 1, explanation: 'ERC-721 only guarantees on-chain ownership via tokenId-to-owner mapping. The actual image and traits come from tokenURI, which typically points to off-chain metadata (often on IPFS) that can become unavailable.' },
        { id: 'proposals_eth_q9', question: 'During a period of heavy network congestion, a user notices the base fee rising sharply block after block. What does this indicate is happening under EIP-1559?', options: ['Validators are manually raising fees to maximize profit', 'Recent blocks have been consistently full, so the protocol increases the base fee by up to 12.5% per block', 'The user is being targeted by a malicious validator', 'ETH issuance has been paused'], correct: 1, explanation: 'EIP-1559 adjusts the base fee algorithmically: a full block raises the next block\'s base fee by up to 12.5%, while an empty block lowers it. Sustained congestion causes consecutive increases.' },
        { id: 'proposals_eth_q10', question: 'A dApp wants new users to interact with it without holding any ETH for gas. Which ERC-4337 component makes this possible, and how?', options: ['The EntryPoint contract, which mints free ETH for new users', 'A paymaster contract that pays gas fees on the user\'s behalf', 'The bundler, which waives gas fees for first-time wallets', 'ERC-20 tokens automatically cover gas costs'], correct: 1, explanation: 'ERC-4337 paymasters are contracts that can sponsor gas fees for UserOperations, letting dApps cover costs for users so they never need to hold ETH themselves.' },
        { id: 'proposals_eth_q11', question: 'A protocol wants exchanges to support its new token immediately at launch with zero custom integration. Which design choice achieves this?', options: ['Writing detailed documentation for exchanges to read', 'Implementing the standard ERC-20 interface so existing wallet and exchange code works unmodified', 'Asking the Ethereum Foundation to whitelist the token', 'Deploying on a testnet first'], correct: 1, explanation: 'Because exchanges and wallets already know how to call the six ERC-20 functions, any contract implementing that interface works immediately, with no custom code required, exactly as Uniswap and Compound do for thousands of tokens.' },
        { id: 'proposals_eth_q12', question: 'A collector buys a Bored Ape and later checks its metadata, only to find the image has changed from what they originally saw. What does this reveal about ERC-721\'s design?', options: ['ERC-721 stores images directly on-chain, so this should be impossible', 'tokenURI metadata is often hosted off-chain, so whoever controls that storage can change what the token displays', 'The token must have been transferred to a different owner', 'ERC-721 contracts are upgradeable by default'], correct: 1, explanation: 'ERC-721 only guarantees that a tokenId maps to one owner on-chain. The actual content comes from tokenURI, frequently pointing to off-chain storage, so if that storage is mutable, the displayed metadata can be swapped after mint.' },
        { id: 'proposals_eth_q13', question: 'A trader notices that during a network attack, ETH circulating supply actually decreased over several days. What mechanism under EIP-1559 explains this?', options: ['Validators voted to burn their staking rewards', 'Heavy transaction volume caused base fees to be burned faster than new ETH was issued from staking', 'The attacker burned ETH to fund the attack', 'A protocol-level emergency burn was triggered'], correct: 1, explanation: 'EIP-1559 burns the base fee on every transaction. During high-activity periods, the burn rate can exceed new ETH issuance from staking rewards, making ETH net deflationary, a phenomenon nicknamed "ultrasound money."' },
        { id: 'proposals_eth_q14', question: 'A user wants their wallet to support social recovery (trusted friends restoring access if a phone is lost) without relying on a centralized custodian. Which proposal enables this and how?', options: ['ERC-20, by adding a recovery field to token contracts', 'ERC-4337, because smart contract wallets can implement custom guardian-based recovery logic', 'EIP-1559, because lower fees make recovery transactions cheaper', 'ERC-721, by minting a recovery NFT'], correct: 1, explanation: 'ERC-4337 turns wallets into smart contracts that can implement arbitrary logic, including designating guardians who can vote to restore access, replacing the all-or-nothing risk of a single private key.' },
        { id: 'proposals_eth_q15', question: 'A bundler receives a UserOperation from a smart wallet under ERC-4337. What does the bundler do with it before it affects Ethereum state?', options: ['It executes it immediately as a standalone special transaction type', 'It packages it with other UserOperations into a regular transaction sent to the EntryPoint contract', 'It stores it permanently as an off-chain record with no on-chain effect', 'It forwards it directly to a validator to be mined as-is'], correct: 1, explanation: 'ERC-4337 requires no core protocol changes: bundlers collect UserOperations from a mempool and bundle them into a normal Ethereum transaction sent to the shared EntryPoint contract, which validates and executes them.' },
      ],
    },
